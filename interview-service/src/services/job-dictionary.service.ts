...
export const fetchActiveDictionary = async (includeInactive = false) => {
  const categories = await withRetry(() => prisma.jobDictionaryCategory.findMany({
...
export const listCategories = async (options: CategoryListOptions = {}) => {
  ...
  const [categories, total] = await withRetry(() => Promise.all([
...
export const listPositions = async (options: PositionListOptions = {}) => {
  ...
  const [positions, total] = await withRetry(() => Promise.all([
...
export const getCategory = async (id: string, includePositions = false) => {
  const category = await withRetry(() => prisma.jobDictionaryCategory.findUnique({
    where: { id },
    include: includePositions
      ? {
          positions: {
...
export const getPosition = async (id: string) => {
  const position = await withRetry(() => prisma.jobDictionaryPosition.findUnique({
    where: { id },
...
export const createCategory = async (payload: {
  ...
  const category = await withRetry(() => prisma.jobDictionaryCategory.create({
    data: {
...
export const updateCategory = async (
  ...
  const category = await withRetry(() => prisma.jobDictionaryCategory.update({
    where: { id },
    data,
  }));
...
export const deleteCategory = async (id: string) => {
  await withRetry(() => prisma.jobDictionaryCategory.delete({
    where: { id },
  }));
};
...
export const createPosition = async (payload: {
  ...
  const position = await withRetry(() => prisma.jobDictionaryPosition.create({
    data: {
      code: payload.code.trim(),
      name: payload.name.trim(),
      description: payload.description ?? null,
...
export const updatePosition = async (
  ...
  const position = await withRetry(() => prisma.jobDictionaryPosition.update({
    where: { id },
    data,
  }));
...
export const deletePosition = async (id: string) => {
  await withRetry(() => prisma.jobDictionaryPosition.delete({
    where: { id },
  }));
};
