...
WeAI::WeAI(int melcnt,int bnfcnt,int trd){
  ...
  bufin = (float*)malloc(countin*sizeof(float)+1024);
  bufout = (float*)malloc(countout*sizeof(float)+1024);
}
...
int WeOnnx::dorun(float* mel,int melcnt,float* bnf,int bnfcnt){
  ...
  Ort::Value arrin[2] = {
    Ort::Value::CreateTensor<float>( memoryInfo, mel ,countin ,  shapein, 3),
    Ort::Value::CreateTensor<int32_t>( memoryInfo, buflen ,countlen ,  shapelen, 1)
  };
  Ort::Value arrout[1] = {
    Ort::Value::CreateTensor<float>( memoryInfo, bnf ,countout ,  shapeout, 3)
  };
