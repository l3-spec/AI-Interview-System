...
/**
 * 全局事件分发器，用于 backend-api 自身广播以及子服务日志的中转广播
 */
...
/**
 * 初始化微服务日志收集与拦截上报机制
 * @param name 微服务名称，如 'backend-api'、'asr-service' 等
 */
export function initServiceLogger(name: string) {
  serviceName = name;

  // 1. 如果是子服务（非 backend-api），尝试连接 Redis，为日志传输建立首选通道
...
async function flushLogs() {
  ...
  try {
    if (serviceName === 'backend-api') {
      // backend-api 直接在本进程内向 emitter 发送事件，完全避免网络开销
      ...
      // 如果 Redis 不可用，降级使用 HTTP POST 上报
      await axios.post(`${backendUrl}/api/system/logs/upload`, {
        serviceName,
        logs: logsToSend,
      }, {
        timeout: 3000,
