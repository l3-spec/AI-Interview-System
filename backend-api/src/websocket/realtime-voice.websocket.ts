...
export class RealtimeVoiceWebSocketServer {
  ...
  private async handleDualTrackStreaming(socket: any, sessionId: string, text: string, context: any) {
     ...
     this.persistAvatarVoice(sessionId, fullResponse);
  }

  /**
   * 核心交互流程处理 (ASR 完成 或 手动文本输入后触发)
   */
  private async handleInteractionFlow(params: {
    sessionId: string;
    text: string;
    source: 'asr' | 'text';
    userId?: string;
    ...
    try {
      ...
      // 容错落入直接对话模式
      console.log(`⚠️ 业务逻辑执行失败，回退到普通 LLM 模式: ${error.message}`);
      const conductorResult = await interviewConductor.generateInterviewerResponse({
        userMessage: text,
        sessionId,
        context: { jobPosition }
      });
      const ttsMode = await this.synthesizeQwen3TtsSegments(ttsSessionId, conductorResult.text);
  ...
  /** 将数字人播报文本写入 AI 面试沟通记录（含情绪标注原文） */
  private persistAvatarVoice(
    sessionId: string,
    ...
    if (!trimmed) return;
    void aiInterviewService
      .recordConversationTurn(sessionId, {
        speaker: 'AVATAR',
        avatarText: trimmed,
  ...
  /** Qwen3-TTS 微服务：按情感分段推送并 commit */
  private async synthesizeQwen3TtsSegments(
    ttsSessionId: string,
    responseText: string,
    scene?: InterviewScene
  ): Promise<'qwen3_streaming' | 'client'> {
    try {
      ...
      console.log(
        `🎙️ [Qwen3-TTS] 开始合成文本流 (Session: ${ttsSessionId}): "${responseText.substring(0, 50)}${responseText.length > 50 ? '...' : ''}"`
      );
  ...
  /**
   * 向 Socket 房间下发面试官题目（与 handleInteractionFlow 一致）；
   * 异步题库就绪后需调用此方法，否则 App 仅收到欢迎语、无正式题。
   */
  private async emitRoundVoiceResponse(params: {
    sessionId: string;
    round: {
      question: string;
      audioUrl?: string;
    ...
    const { sessionId, round, ttsSessionId: rawTts } = params;
    const ttsSessionId = rawTts || sessionId;

    let ttsMode: string = 'client';
    if (round.audioUrl) {
      ttsMode = 'server';
      console.log(`🔊 [VoiceResponse] 生成语音包 URL (下一步问题): ${round.audioUrl}`);
    } else {
      const scene = interviewConductor.inferScene(round.question, {
        isFollowUp: (round.followupCount || 0) > 0,
      });
      ttsMode = await this.synthesizeQwen3TtsSegments(ttsSessionId, round.question, scene);
      if (ttsMode === 'qwen3_streaming') {
        console.log(`🎤 [VoiceResponse] 使用 Qwen3-TTS 流式播报 (Session: ${ttsSessionId})`);
      }
    }
      ...
      if (asrServiceUrl || ttsServiceUrl) {
        ...
        if (asrHealth?.status === 'ok' || ttsHealth?.status === 'ok') {
          ...
          qwen3ASRClient.on('transcription_completed', async (event) => {
            ...
            console.log(`[Qwen3] ASR 识别完成 (session: ${sessionId}): "${text}"`);
            
            // 自动触发消息处理流程
            await this.handleInteractionFlow({
              sessionId,
              text,
              source: 'asr'
            }).catch(err => {
              console.error(`[Qwen3] 自动处理 ASR 结果失败: ${err.message}`);
            });
          });
    ...
    this.io.on('connection', (socket) => {
      ...
      socket.on('join_session', async (data: {
        ...
        try {
          ...
          // 优先尝试使用 Qwen3-TTS 微服务进行流式播报（推荐）
          const ttsHealth = await qwen3TTSClient.checkHealth();
          if (ttsHealth?.status === 'ok') {
            console.log(`🎤 使用 Qwen3-TTS 微服务发送欢迎语 - sessionId: ${sessionId}`);
            
            const segments = interviewConductor['parseEmotionSegments'](welcomeText, isResume ? 'resume' : 'welcome' as any);
            for (const segment of segments) {
            ...
            try {
              ...
              console.warn(`⚠️ 后端生成欢迎语音频失败，退回到客户端原生 TTS:`, ttsError);
              socket.emit('voice_response', {
                audioUrl: null,
                text: welcomeText,
                sessionId,
            ...
            .then(async (res) => {
              ...
              if (res.totalRounds > 0 && res.firstRound) {
                try {
                  ...
                  console.warn(`⚠️ [Coordinator] 首题下发失败: ${pushErr?.message || pushErr}`);
                }
              }
            })
            .catch((err) => {
      ...
      socket.on('text_message', async (data: {
        ...
        try {
          ...
          await this.handleInteractionFlow({
            ...
            ttsSessionId: data.ttsSessionId
          });
        } catch (error: any) {
          console.error('处理文本消息失败:', error);
          socket.emit('error', { message: error.message || '处理失败' });
        }
      });
